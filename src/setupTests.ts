...
jest.mock('@chakra-ui/icons', () => {
    ...
    const makeIcon = (testId: string) => (props: any) => React.createElement('span', { ...props, 'data-testid': testId });
    return {
...
jest.mock('@chakra-ui/react', () => {
    ...
    const filterChakraProps = (props: any) => {
        ...
        const chakraProps = new Set([
            ...
            // Flexbox props
            'flex', 'flexDir', 'flexWrap', 'flexGrow', 'flexShrink', 'flexBasis',
            'justify', 'justifyItems', 'justifyContent', 'justifySelf',
            'align', 'alignItems', 'alignContent', 'alignSelf',
            // Grid props
            'grid', 'gridArea', 'gridAutoColumns', 'gridAutoFlow', 'gridAutoRows',
            'gridColumn', 'gridColumnEnd', 'gridColumnGap', 'gridColumnStart',
            'gridGap', 'gridRow', 'gridRowEnd', 'gridRowGap', 'gridRowStart',
            'gridTemplate', 'gridTemplateAreas', 'gridTemplateColumns', 'gridTemplateRows',
            // Position props
            'pos', 'top', 'right', 'bottom', 'left', 'zIndex',
            // Display props
            'd', 'display', 'overflow', 'overflowX', 'overflowY',
            // Visual props
            'bg', 'bgColor', 'background', 'backgroundImage', 'backgroundSize',
            'backgroundPosition', 'backgroundRepeat', 'backgroundAttachment',
            'backgroundColor', 'bgGradient', 'bgClip',
            'color', 'opacity', 'boxShadow', 'textShadow', 'borderRadius',
            'border', 'borderWidth', 'borderStyle', 'borderColor',
            'borderTop', 'borderRight', 'borderBottom', 'borderLeft',
            // Typography props
            'fontFamily', 'fontSize', 'fontWeight', 'lineHeight', 'letterSpacing',
            'textAlign', 'textDecoration', 'textTransform', 'textOverflow',
            'whiteSpace', 'wordBreak', 'noOfLines',
            // Other Chakra props
            'isInvalid', 'isDisabled', 'isReadOnly', 'isRequired',
            'variant', 'size', 'colorScheme', '_hover', '_focus', '_active',
            '_before', '_after', '_placeholder', '_selection',
            '_groupHover', '_groupFocus', '_groupActive',
            // Box props
            'boxSize', 'minHeight', 'minWidth', 'maxHeight', 'maxWidth',
            // Motion props
            'animate', 'initial', 'exit', 'transition', 'whileHover', 'whileTap', 'whileInView',
            'variants', 'viewport',
            // Other problematic props
            'backdropFilter', 'isOpen', 'transformOrigin', 'objectFit', 'objectPosition',
            'paddingLeft', 'paddingRight', 'isLoading', 'loadingText', 'leastDestructiveRef',
            'fallback', 'spacing', 'columns', 'placement', 'icon'
        ]);
    ...
    const el = (tag: string) => (props: any) => React.createElement(tag, filterChakraProps(props), props.children);
    ...
    const findNextFormElement = (children: any, labelIndex: number): any => {
        ...
        for (let i = labelIndex + 1; i < childrenArray.length; i++) {
            ...
            if (child && typeof child === 'object' && 'type' in child && child.type) {
                ...
                if (componentName === 'Input' || componentName === 'Select' || componentName === 'Textarea') {
                    return child;
                }
            }
        }
    ...
    return {
        ...
        useBreakpointValue: (values: any) => (values && (values.base ?? values.sm ?? values.md ?? values.lg)) ?? undefined,
...
jest.mock('framer-motion', () => {
    ...
    const filterMotionProps = (props: any) => {
        ...
        const motionProps = new Set([
            'animate', 'initial', 'exit', 'transition', 'whileHover', 'whileTap', 'whileInView',
            'whileFocus', 'whileDrag', 'whileDragStart', 'whileDragEnd',
    ...
    ['div', 'span', 'a', 'img', 'h1', 'h2', 'h3', 'p', 'button'].forEach((tag) => {
        motion[tag] = (props: any) => React.createElement(tag, filterMotionProps(props), props.children);
    });
